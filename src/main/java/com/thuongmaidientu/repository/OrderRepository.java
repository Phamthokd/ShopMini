...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query("SELECT o FROM Order o " + "JOIN o.orderDetails od " + "JOIN od.product p " + "WHERE od.status = :status "
	...
	@Query(value = "SELECT DATE(o.createdDate) AS orderDate, SUM(od.unitPrice * od.quantity) AS totalRevenue "
			+ "FROM Order o " + "JOIN o.orderDetails od " + "WHERE od.product.userProduct = :user "
			+ "GROUP BY DATE(o.createdDate)")
	List<Object[]> calculateTotalRevenueByDayForSeller(@Param("user") User user);

	@Query("SELECT o FROM Order o " + "WHERE MONTH(o.createdDate) = MONTH(CURRENT_DATE) ")
